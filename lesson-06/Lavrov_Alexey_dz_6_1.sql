/* Lavrov Alexey. DZ-6. 
 * 1. Пусть задан некоторый пользователь. Из всех пользователей соц. сети найдите человека,A
...
-- находим id пользователей, которые отправили максимальное кол-во сообщений "избранному"
SELECT COUNT(*) AS count_messages, from_user_id FROM messages WHERE to_user_id = @choosen_user
    GROUP BY from_user_id 
    HAVING count_messages =
        (SELECT COUNT(*) AS count_messages FROM messages 
        WHERE to_user_id = @choosen_user 
        GROUP BY from_user_id 
        ORDER BY count_messages 
...
-- собираем данные о пользователях из таблицы users
SELECT 
    id,
    first_name,
    last_name,
    email,
    phone
    FROM users WHERE id IN (1, 9);

-- собираем данные о пользователях из таблицы profiles
   SELECT user_id,
        CASE(gender)
            WHEN 'm' THEN 'male'
            WHEN 'f' THEN 'female'
        ...
        city,
        country
        FROM profiles WHERE user_id IN (1, 9);

-- соединяем данные о пользователях из users и profiles
SELECT 
    first_name,
    last_name,
    (SELECT 
    ...
    (SELECT country
    	FROM profiles WHERE profiles.user_id = users.id) AS country,
    (SELECT city
    	FROM profiles WHERE profiles.user_id = users.id) AS city,
	email,
    phone
    FROM users WHERE id IN (1, 9);
    
-- теперь объединим оба запроса - поиск id пользователей, больше всех 
-- писавших нашему "избранному" и вывод инфы по этим пользователям
-- через JOIN явно было бы короче и "прозрачнее", но их еще не проходили...
SELECT 
	COUNT(*) AS count_messages, 
	(SELECT CONCAT(last_name, ' ', first_name) 
		FROM users WHERE id = messages.from_user_id) AS user_name,
	(SELECT 
    	CASE(gender) 
			WHEN 'f' THEN 'female'
			WHEN 'm' THEN 'male'
		...
		END AS gender 
		FROM profiles 
		WHERE user_id = messages.from_user_id) AS gender,
	(SELECT country FROM profiles 
		WHERE user_id = messages.from_user_id) AS country,
	(SELECT city FROM profiles 
		WHERE user_id = messages.from_user_id) AS city,
	(SELECT email FROM users 
		WHERE id = messages.from_user_id) AS email,
	(SELECT phone FROM users 
		WHERE id = messages.from_user_id) AS phone
